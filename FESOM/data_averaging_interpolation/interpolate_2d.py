...
# /work/ab0995/a270088/NextGems/Cycle3/weights/weights_FESOM_tco2559-ng5_original_2d_ycon_ecmwf_{args.resolution}_l2d.nc
# /work/ab0995/a270088/NextGems/Cycle3/weights/weights_FESOM_tco2559-ng5_original_2d_ycon_ecmwf_{args.resolution}_l2d_elements.nc

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process some arguments.")
    parser.add_argument("--experiment", type=str, required=True, help="Experiment name")
    parser.add_argument(
        "--what_interpolate", type=str, required=True, help="Interpolation type"
    )
    parser.add_argument(
    ...
    parser.add_argument("--node_weights", type=str, required=True, help="node weights")
    parser.add_argument(
        "--elem_weights", type=str, required=True, help="element weights"
    )
    ...
    regridder = rg.Regridder(weights=f"{args.node_weights}")
    ...
    for var_name, da in data.data_vars.items():
        opath = os.path.join(
            ...
            args.experiment,
            args.resolution,
            args.what_interpolate,
            var_name,
        )
        ensure_directory_exists(opath)
        if var_name in ["tx_sur", "ty_sur"]:
            regridder = rg.Regridder(weights=f"{args.elem_weights}")
        for year in range(args.start_year, args.end_year + 1):
            for mon in range(1, 13):
                print(f"{var_name}, {year}, {mon}")
                ofile = os.path.join(
                    opath, f"{var_name}_{str(year)}-{str(mon).zfill(2)}.nc"
                )
                if not check_file_exists(ofile):
                    monthly_interp = regridder.regrid(
                        da.sel(time=f"{str(year)}-{str(mon).zfill(2)}")
                    )
                    monthly_interp = monthly_interp.to_dataset()
                    monthly_interp.attrs = {
                        "model": "FESOM2",
                        "experiment": args.experiment,
                        "var_type": args.what_interpolate,
                        "target_resolution": args.resolution,
                    }
                    monthly_interp.to_netcdf(
                        ofile,
                        encoding={
                            var_name: {
                                "zlib": True,
